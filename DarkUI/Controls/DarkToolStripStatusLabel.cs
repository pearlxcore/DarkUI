...
{
    /// <summary>
    /// A themed ToolStripStatusLabel. Colours are applied once the control is
    /// hosted on a ToolStrip (OnOwnerChanged), never in the constructor.
    /// </summary>
    public class DarkToolStripStatusLabel : ToolStripStatusLabel
    {
        private bool _disposed;

        public DarkToolStripStatusLabel() : base()
        {
            ThemeManager.ThemeChanged += OnThemeChanged;
        }

        public DarkToolStripStatusLabel(string text) : base(text)
        {
            ThemeManager.ThemeChanged += OnThemeChanged;
        }

        public DarkToolStripStatusLabel(string text, Image image) : base(text, image)
        {
            ThemeManager.ThemeChanged += OnThemeChanged;
        }
