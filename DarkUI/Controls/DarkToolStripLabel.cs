...
{
    /// <summary>
    /// A themed ToolStripLabel. Colours are applied once the control is
    /// hosted on a ToolStrip (OnOwnerChanged), never in the constructor.
    /// </summary>
    public class DarkToolStripLabel : ToolStripLabel
    {
        private bool _disposed;

        public DarkToolStripLabel() : base()
        {
            ThemeManager.ThemeChanged += OnThemeChanged;
        }

        public DarkToolStripLabel(string text) : base(text)
        {
            ThemeManager.ThemeChanged += OnThemeChanged;
        }

        public DarkToolStripLabel(string text, Image image) : base(text, image)
        {
            ThemeManager.ThemeChanged += OnThemeChanged;
        }
